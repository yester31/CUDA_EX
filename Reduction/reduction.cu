...
// reduction 2
__global__ void reduction_2_kernel(long* output, long* input, int num_elements)
{
	extern __shared__ long shared_data[];
	// each thread loads one element from global to shared mem
	unsigned int tid = threadIdx.x;
...
void reduction_2_gpu(std::vector<long>& output_gpu, std::vector<long>& input)
{
	...
	reduction_2_kernel << <dimGrid, dimBlock, block * sizeof(long) >> > (dev_o, dev_i, thread_cnt);
...
// reduction 3
__global__ void reduction_3_kernel(long* output, long* input, int num_elements)
{
	extern __shared__ long shared_data[];
	// each thread loads one element from global to shared mem
	unsigned int tid = threadIdx.x;
	unsigned int index = threadIdx.x + (blockIdx.x * 2) * blockDim.x;
	if (index >= num_elements) return;
	shared_data[tid] = input[index] + input[index + blockIdx.x];
...
void reduction_3_gpu(std::vector<long>& output_gpu, std::vector<long>& input)
{
	...
	reduction_3_kernel << <dimGrid, dimBlock, block * sizeof(long) >> > (dev_o, dev_i, thread_cnt);
